...
class Collator(object):
    ...
    def __call__(self, batch_idx):
        ...
        batch_input_ids = np.zeros((batch_size, self.max_seq_len), dtype=np.float64)
        batch_token_type_ids = np.ones((batch_size, self.max_seq_len), dtype=np.float64)

        batch_y_start = np.zeros((batch_size,), dtype=np.float64)
        batch_y_end = np.zeros((batch_size,), dtype=np.float64)
        batch_y = np.zeros((batch_size,), dtype=np.float64)

        x=torch.DoubleTensor(batch_y_start)
        y=torch.DoubleTensor(batch_y_end)
        z=torch.DoubleTensor(batch_y)
        x.requires_grad_()
        y.requires_grad_()
        z.requires_grad_()
        ...
        return torch.from_numpy(batch_input_ids), torch.from_numpy(batch_attention_mask), torch.from_numpy(batch_token_type_ids),x,y,z
...
def modelsize(model, input, type_size=4):
 ...
 # coding=gbk
    ...
    print('Model {} : params: {:4f}M'.format(model._get_name(), para * type_size / 1000 / 1000),flush=True)
    ...
    print('Model {} : intermedite variables: {:3f} M (without backward)'
          .format(model._get_name(), total_nums * type_size / 1000 / 1000),flush=True)
    print('Model {} : intermedite variables: {:3f} M (with backward)'
          .format(model._get_name(), total_nums * type_size*4 / 1000 / 1000),flush=True)
    # coding=UTF-8
...
def main():
    ...
    # prepare input
    import pickle
    with open('/nfsshare/home/dl08/Dan/TFQA1/code/1_1/distribution_dict1.pickle', 'rb') as f:
        distribution_dict1 = pickle.load(f)  
    with open('/nfsshare/home/dl08/Dan/TFQA1/code/1_1/distribution_dict2.pickle', 'rb') as f:
        distribution_dict2 = pickle.load(f)
    with open('/nfsshare/home/dl08/Dan/TFQA1/code/1_1/distribution_dict3.pickle', 'rb') as f:
        distribution_dict3 = pickle.load(f)
    with open('/nfsshare/home/dl08/Dan/TFQA1/code/1_1/distribution_dict4.pickle', 'rb') as f:
        distribution_dict4 = pickle.load(f)

    json_dir = '/nfsshare/home/dl08/Dan/TFQA1/input/simplified-nq-train.jsonl'
    max_data = 9999999999
    ...
    print("bert-large-uncased_4 train_epoch0 pre input is finished!", flush=True)
    ...
    model_path = '/nfsshare/home/dl08/Dan/TFQA1/code/bert-large-uncased_4/model'
    ...
    print('We have added', num_added_toks, 'tokens', flush=True)
    ...
    print('Model {} : params: {:4f}M'.format(model._get_name()£¬ para * 4 / 1000 / 1000),flush=True)
    ...
    print("bert-large-uncased_4 train_epoch0 build model is finished!", flush=True)
    ...
    model.train()
    print("model.train() started", flush=True)
    for j,(batch_input_ids, batch_attention_mask, batch_token_type_ids, batch_y_start, batch_y_end, batch_y) in enumerate(train_generator):
        ...
        batch_input_ids.requires_grad_()
        ######### no grad #########
        with torch.no_grad():
        
            modelsize(model,batch_input_ids , type_size=8)        
        
            logits1, logits2, logits3 = model(batch_input_ids, batch_attention_mask, batch_token_type_ids)
            

        print("setting no grad", flush=True)
        ###########################
    ...
    print("bert-large-uncased_4 train_epoch0 train is finished!", flush=True)

if __name__ == "__main__":
    main()
    print("bert-large-uncased_4 train_epoch0 is finished!", flush=True)
