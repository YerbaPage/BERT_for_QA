...
class Collator(object):
    ...
    def __call__(self, batch_idx):
        ...
        x=torch.LongTensor(batch_y_start)
        y=torch.LongTensor(batch_y_end)
        z=torch.LongTensor(batch_y)
...
def main():
    ...
    for j,(batch_input_ids, batch_attention_mask, batch_token_type_ids, batch_y_start, batch_y_end, batch_y) in enumerate(train_generator):
        ...
        batch_attention_mask.requires_grad_()
        batch_token_type_ids.requires_grad_()
        ######### no grad #########
        with torch.no_grad():
