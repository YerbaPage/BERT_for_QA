...
def main():
    ...
    for j,(batch_input_ids, batch_attention_mask, batch_token_type_ids, batch_y_start, batch_y_end, batch_y) in enumerate(train_generator):
        ...
        ##############
        print("losses1 :", losses1, flush = True)
        losses1list.append(losses1.avg)
        losses2list.append(losses2.avg)
        losses3list.append(losses3.avg)
        accuracies1list.append(accuracies1.avg)
        accuracies2list.append(accuracies2.avg)
        accuracies3list.append(accuracies3.avg)
        ##############
    ...
    ###############
    np.save("losses1.npy" ,np.array(losses1list))
    np.save("losses2.npy" ,np.array(losses2list))
    np.save("losses3.npy" ,np.array(losses3list))
    np.save("accuracies1.npy" ,np.array(accuracies1list))
    np.save("accuracies2.npy" ,np.array(accuracies2list))
    np.save("accuracies3.npy" ,np.array(accuracies3list))
    ###############
