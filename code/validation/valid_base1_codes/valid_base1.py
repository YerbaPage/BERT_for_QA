...
def reduce1(n_candidate=10, th_candidate=0.2):
    ...
    model = BertForQuestionAnswering.from_pretrained('../albert-xxlarge-v2_4/weights/epoch2/', config=config)
...
print("before predicting", flush=True)

model_dir = '../bert-base-uncased_1/weights/epoch2/'
test_prob1, test_prob2, test_prob3 = bert_large_predict(data_dict, id_list, id_candidate_len_dict, id_candidate_list_sorted, model_dir, word_len)
...
################
np.save("start_prob_6-11", start_prob, allow_pickle=True, fix_imports=True)
np.save("end_prob_6-11", end_prob, allow_pickle=True, fix_imports=True)
np.save("class_prob_6-11", class_prob, allow_pickle=True, fix_imports=True)
################
...
##############
print("before final saving", flush=True)
np.save("temp_dict_6-11", temp_dict)
np.save("final_dict_6-11", final_dict)
##############
