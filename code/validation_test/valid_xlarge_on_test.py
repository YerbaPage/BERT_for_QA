...
def reduce1(n_candidate=10, th_candidate=0.2):
    ...
    # prepare input
    json_dir = '../../input/simplified-nq-test.jsonl'
...
def albert_predict(data_dict, id_list, id_candidate_len_dict, id_candidate_list_sorted, model_dir, word_len):
    ...
    # build model
    model_path = '../albert-xxlarge-v2_4/model/'
...
print("before albert predicting", flush=True)

###############

np.save("data_dict_test", data_dict)
np.save("id_list_test", id_list)
np.save("id_candidate_len_dict_test", id_candidate_len_dict)
np.save("id_candidate_list_sorted_test", id_candidate_list_sorted)

###############

model_dir = '../albert-xxlarge-v2_4/weights/epoch2/'
test_prob1, test_prob2, test_prob3 = albert_predict(data_dict, id_list, id_candidate_len_dict, id_candidate_list_sorted, model_dir, word_len)
start_prob += test_prob1
end_prob += test_prob2
class_prob += test_prob3

print("before saving", flush=True)
################
np.save("start_prob_test", start_prob, allow_pickle=True, fix_imports=True)
np.save("end_prob_test", end_prob, allow_pickle=True, fix_imports=True)
np.save("class_prob_test", class_prob, allow_pickle=True, fix_imports=True)
################
...
##############
print("before final saving_test", flush=True)
np.save("temp_dict_test", temp_dict)
np.save("final_dict_test", final_dict)
##############
print("after final saving", flush=True)
# dump to json
with open('predictions_test_albert_epoch2.json', 'w') as fp:
